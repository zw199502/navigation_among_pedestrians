...
class Navigation:
  ...
  def __init__(self, config, mode):
    ...
    self._passage_width = config.passage_width
    ...
    self._passage_width_grid = self._passage_width / self._map_resolution
  ...
  def outside_check(self, map_h, map_w, grid_radius, inflation=False):
    ...
    if self._scenario == 'quadruped_loam':
      # motion area is within a long but narrow passage
      if map_w < (self._size[0] - self._passage_width_grid) / 2.0 \
         or map_w >= (self._size[0] + self._passage_width_grid) / 2.0 \
            or map_h - delta_grid < 0 or map_h + delta_grid >= self._size[0]:
        return True
      else:
        return False
    else:
      if map_w - delta_grid < 0 or map_w + delta_grid >= self._size[0] or map_h - delta_grid < 0 or map_h + delta_grid >= self._size[0]:
        return True
      else:
        return False
  ...
  def step(self, action, full_state=False):
    ...
    _action = np.zeros(2)
    _action[0] = self._max_robot_speed * action['action'][0]
    _action[1] = self._max_robot_sidespeed * action['action'][1]
    ...
    if self._real_world:
      ...
      self._robot.update_states(self._robot_pose[0], self._robot_pose[1], self._robot_pose[2], _action)
      ...
      for i in range(self._human_num):
        ...
        human_out = self.outside_check(human_map_h, human_map_w, self._humans_grid_raidus[i])
        if human_out:
          human_outside = True
          break
    ...
    if not self._real_world:
      for i, human in enumerate(self._humans):
        ...
        if human.reached_destination():
          if self._scenario == 'quadruped_loam':
            del self._humans[i]
            del self._humans_grid_area[i]
            del self._humans_grid_raidus[i]
            add_human = np.random.randint(2, size=1)[0]
            if add_human == 1:
              human_new, grid_area, grid_radius = self.generate_circle_crossing_human()
              self._humans.append(human_new)
  ...
  def generate_random_humans(self):
    ...
    for i in range(self._human_num):
      human, grid_area, grid_radius = self.generate_circle_crossing_human()
  ...
  def generate_circle_crossing_human(self):
    ...
    while True:
      if self._scenario == 'quadruped_loam':
        px = np.random.uniform(-self._circle_radius, self._circle_radius, 1)[0]
        py = np.random.uniform(-0.8, 0.8, 1)[0]
        direction = np.random.randint(2, size=1)[0]
        if direction == 0:
          gx = self._circle_radius
        else:
          gx = -self._circle_radius
        gy = np.random.uniform(-0.8, 0.8, 1)[0]
    ...
    return human, grid_area, grid_radius
  ...
  def reset(self, full_state=False):
    if self._real_world:
      ...
      # px, py, gx, gy, vx, vy, theta
      self._robot.set(-self._circle_radius, 0, self._circle_radius, 0, 0, 0, 0)
      if self._random_robot:
        ### robot position and goal are randomly set on a square
        if self._scenario == 'quadruped_motion_capture':
          self._robot.px = np.random.uniform(-self._circle_radius, -0.5, 1)[0]
          self._robot.py = np.random.uniform(-self._circle_radius, self._circle_radius, 1)[0]
          self._robot.gx = np.random.uniform(0.5, self._circle_radius, 1)[0]
          self._robot.gy = np.random.uniform(-self._circle_radius, self._circle_radius, 1)[0]
        elif self._scenario == 'quadruped_loam': # passage environment, smaller area
          self._robot.px = np.random.uniform(-self._circle_radius, -0.5, 1)[0]
          self._robot.py = np.random.uniform(-0.8, 0.8, 1)[0]
          self._robot.gx = np.random.uniform(0.5, self._circle_radius, 1)[0]
          self._robot.gy = np.random.uniform(-0.8, 0.8, 1)[0]
        else:
          raise NotImplementedError(self._scenario)
