...
class Navigation:
  ...
  def __init__(self, config, mode):
    ...
    self._max_robot_sidespeed = config.max_robot_sidespeed
    ...
    if self._scenario == 'simulation':
      # this scenario is used for comparing with ORCA, SARL, RGL, CADRL, LSTM_RL and EGO in simulation
      self._circle_radius = self._harf_area - 1.0
    else:
      # this in real world scenario
    ...
    if self._real_world:
      ...
      self._pub_cmd_vel = rospy.Publisher('cmd_vel', Twist, queue_size=5)
      if self._scenario == 'real_robot_external_localization':
        # get pose from the motion capture system
        sub_robot_pose = rospy.Subscriber("/vrpn_client_node/QuadrupedRobot/pose", PoseStamped, self.robot_pose_callback)
        sub_person1_pose = rospy.Subscriber("/vrpn_client_node/person1/pose", PoseStamped, self.person1_pose_callback)
        sub_person2_pose = rospy.Subscriber("/vrpn_client_node/person2/pose", PoseStamped, self.person2_pose_callback)
        sub_person3_pose = rospy.Subscriber("/vrpn_client_node/person3/pose", PoseStamped, self.person3_pose_callback)
      elif self._scenario == 'real_robot_loam':
        ...
        sub_person1_pose = rospy.Subscriber("/person1/pose", PoseStamped, self.person1_pose_callback)
        sub_person2_pose = rospy.Subscriber("/person2/pose", PoseStamped, self.person2_pose_callback)
        sub_person3_pose = rospy.Subscriber("/person3/pose", PoseStamped, self.person3_pose_callback)
      else:
    ...
    self._arrival_time = self._time_limit * self._time_step
  ...
  def robot_pose_callback(self, msg):
    ...
    if self._scenario == 'real_robot_external_localization':
      # transform the original position to keep the goal position constant as [1.0, 0.0]
      self._robot_pose[0] = position.x - 0.7
    elif self._scenario == 'real_robot_loam':
  ...
  def person1_pose_callback(self, msg):
    ...
    if self._scenario == 'real_robot_external_localization':
      # transform the original position to keep the goal position constant as [1.0, 0.0]
      self._position_person1[0] = position.x - 0.7
    elif self._scenario == 'real_robot_loam':
  ...
  def person2_pose_callback(self, msg):
    ...
    if self._scenario == 'real_robot_external_localization':
      # transform the original position to keep the goal position constant as [1.0, 0.0]
      self._position_person2[0] = position.x - 0.7
    elif self._scenario == 'real_robot_loam':
  ...
  def person3_pose_callback(self, msg):
    ...
    if self._scenario == 'real_robot_external_localization':
      # transform the original position to keep the goal position constant as [1.0, 0.0]
      self._position_person3[0] = position.x - 0.7
    elif self._scenario == 'real_robot_loam':
  ...
  def get_image(self):
    ...
    for i in range(self._human_num):
      human_margin = self._harf_area - self._human_radius + 0.1
      if self._humans[i].px < human_margin and self._humans[i].px > -human_margin \
        and self._humans[i].py < human_margin and self._humans[i].py > -human_margin:  # humans with far location are fake
    ...
    # OpenCV is BGR
    goal_map_zero = np.zeros((self._size[0], self._size[1], 1), dtype=np.uint8)
  ...
  def step(self, action, full_state=False):
    ...
    _action = np.zeros(2)
    _action[0] = action['action'][0] * self._max_robot_speed
    _action[1] = action['action'][1] * self._max_robot_sidespeed

    if self._real_world:
      human_outside = False
      # because of the drift and initial error in the yaw roration axis
      # the original command should be transformed to the speed command related to the robot frame
      ...
      self._posititon_saving['action'].append(_action)
      self._posititon_saving['person1'].append(np.array([self._humans[0].px, self._humans[0].py]))
      self._posititon_saving['person2'].append(np.array([self._humans[1].px, self._humans[1].py]))
      self._posititon_saving['person3'].append(np.array([self._humans[2].px, self._humans[2].py]))
    ...
    goal_dis_map = hypot(goal_map_h - robot_map_h, goal_map_w - robot_map_w) * self._map_resolution
    goal_reach = (goal_dis_map <= self._initial_min_goal_dis) 
    goal_dis_real = self._robot.get_goal_distance()
    goal_reach_larger_area = goal_dis_real < self._goal_reach_larger_dis
    
    if goal_reach_larger_area and (not self._goal_reach_larger_area):
      self._goal_reach_larger_area = True
      self._arrival_time = (self._current_step + 1) * self._time_step
      print('goal reaching on larger area at step: ', self._current_step)
    ...
    if human_outside:
      print('human outside')
      # is_last = True
    if self._current_step % self._time_limit == 0 and self._real_world == False:
      print('time out')
      is_last = True
    ...
    if not self._real_world:
      if goal_reach:
        ...
        reward = self._collision_reward_factor * discomfort_grid_value / 255.0 + \
                self._goal_reward_factor * (1.0 - goal_dis_map / self._motion_area)

    if (self._goal_reach_larger_area and self._real_world) or (is_last and self._real_world):
      # stop the robot
      move_cmd = Twist()
      ...
      if not self._saved:
        self._saved = True
        posititon_saving_temp = {}
        for key in self._posititon_saving.keys():
          posititon_saving_temp[key] = np.array(self._posititon_saving[key])
          np.savez_compressed('positions.npz', **posititon_saving_temp)
    ...
    obs = {
        ...
        'image': self._image,
        'goal_reach_larger_area': self._goal_reach_larger_area or goal_reach,
  ...
  def generate_random_humans(self):
    ...
    if self._random_human_num:
      self._human_num = np.random.randint(self._human_num_max, size=1)[0] + 1
    else:
  ...
  def generate_circle_crossing_human(self):
    ...
    while True:
      if self._scenario == 'simulation':
        ...
        position = np.random.uniform(-self._circle_radius, self._circle_radius, 2)
        px = position[0]
        py = position[1]
        goal_margin = np.random.randint(4, size=1)[0]
        margin_value = self._circle_radius - 0.0
        if goal_margin == 0:
          gx = margin_value
          gy = np.random.uniform(-margin_value, margin_value, 1)[0]
        elif goal_margin == 1:
          gx = -margin_value
          gy = np.random.uniform(-margin_value, margin_value, 1)[0]
        elif goal_margin == 2:
          gy = margin_value
          gx = np.random.uniform(-margin_value, margin_value, 1)[0]
        else:
          gy = -margin_value
          gx = np.random.uniform(-margin_value, margin_value, 1)[0]
      ...
      for agent in [self._robot] + self._humans:
        ...
        if self._scenario == 'simulation':
          ...
          if hypot(px - agent.px, py - agent.py) < min_dist or \
              hypot(gx - agent.gx, gy - agent.gy) < min_dist:
            collide = True
            break
  ...
  def reset(self, full_state=False):
    if self._real_world:
      ...
      # add some fake persons whose x and y positions are more than 999 or any large values
      self._human_num = 3  
      self._humans = []
      self._humans_grid_area = []
      self._humans_grid_raidus = []
      ...
      self._posititon_saving['robot'].append(np.array([self._robot.px, self._robot.py]))
      self._posititon_saving['action'].append(np.zeros(2))
      self._posititon_saving['person1'].append(np.array([self._humans[0].px, self._humans[0].py]))
      self._posititon_saving['person2'].append(np.array([self._humans[1].px, self._humans[1].py]))
      self._posititon_saving['person3'].append(np.array([self._humans[2].px, self._humans[2].py]))
      ...
      if self._random_robot:
        if self._scenario == 'real_robot_external_localization' or self._scenario == 'real_robot_loam':
          self._robot.gx = self._goal_position[0]
          self._robot.gy = self._goal_position[1]
          self._robot.px = np.random.uniform(-self._circle_radius, 0.0, 1)[0]
          self._robot.py = np.random.uniform(-self._circle_radius + 0.5, self._circle_radius - 0.5, 1)[0]
        else:
          raise NotImplementedError(self._scenario)
