...
class Agent(common.Module):
  ...
  def __init__(self, config, act_space, step):
    ...
    self.wm = WorldModel(config, self.tfstep)
  ...
  def report(self, data):
    ...
    report['openl_decoder'] = self.wm.video_pred(data, 'image')
    return report


class WorldModel(common.Module):

  def __init__(self, config, tfstep):
    self.config = config
    self.tfstep = tfstep
    self.rssm = common.EnsembleRSSM(**config.rssm)
    if config.render_size[0] == 128:
      self.encoder = common.EncoderLarge()
    else:
      self.encoder = common.Encoder()
    self.heads = {}
    if config.render_size[0] == 128:
      self.heads['image'] = common.DecoderLarge()
    else:
      self.heads['image'] = common.Decoder()
  ...
  def video_pred(self, data, key):
    decoder = self.heads['image']
    truth = data[key][:6] + 0.5
    embed = self.encoder(data)
    states, _ = self.rssm.observe(
        embed[:6, :5], data['action'][:6, :5], data['is_first'][:6, :5])
    recon = decoder(self.rssm.get_feat(states)).mode()[:6]
    init = {k: v[:, -1] for k, v in states.items()}
    prior = self.rssm.imagine(data['action'][:6, 5:], init)
    openl = decoder(self.rssm.get_feat(prior)).mode()
