...
class CrowdSim(gym.Env):
    ...
    def generate_human(self, human=None):
        ...
        if self.randomize_attributes:
            human.v_pref = np.random.uniform(self.robot.v_pref / 2.0, self.robot.v_pref, 1)[0]

        if self.current_scenario == 'circle_crossing':
            while True:
                if self.randomize_attributes:
                    position = np.random.uniform(-self.circle_radius, self.circle_radius, 2)
                    px = position[0]
                    py = position[1]
                    goal_margin = np.random.randint(4, size=1)[0]
                    margin_value = self.circle_radius - 0.0
                    if goal_margin == 0:
                        gx = margin_value
                    ...
                    px_noise = (np.random.random() - 0.25) * human.v_pref
                    py_noise = (np.random.random() - 0.25) * human.v_pref
    ...
    def reset(self, phase='test', test_case=None):
        ...
        if self.randomize_attributes:
            self.robot.gx = 0.0
            self.robot.gy = 1.0
            self.robot.px = np.random.uniform(-self.circle_radius + 0.5, self.circle_radius - 0.5, 1)[0]
            self.robot.py = np.random.uniform(-self.circle_radius, 0.0, 1)[0]
        ...
        self.humans = []
        if self.randomize_attributes:
            self.human_num_random = np.random.randint(self.human_num, size=1)[0] + 1
            for _ in range(self.human_num_random):
                self.humans.append(self.generate_human())
        else:
            for _ in range(human_num):
                self.humans.append(self.generate_human())
        ...
        if self.centralized_planning:
            ...
            self.centralized_planner.max_speed = self.robot.v_pref
            self.centralized_planner.radius = self.humans[0].radius
    ...
    def step(self, action, update=True):
        ...
        if update:
            ...
            for human, action in zip(self.humans, human_actions):
                human.step(action)
                if not self.randomize_attributes:
