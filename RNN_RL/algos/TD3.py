...
class Actor(nn.Module):
    def __init__(
        self, lidar_state_dim, position_state_dim, lidar_feature_dim, action_dim, hidden_dim, max_action, is_recurrent=False
    ):
...
class Critic(nn.Module):
    def __init__(
        self, lidar_state_dim, position_state_dim, lidar_feature_dim, action_dim, hidden_dim, is_recurrent=False
    ):
        ...
        if self.recurrent:
            ...
            self.l4 = nn.Linear(lidar_feature_dim + position_state_dim + action_dim, hidden_dim)
...
class TD3(object):
    ...
    def select_action(self, lidar_state, position_state, hidden, test=True):
        if self.recurrent:
            ...
            lidar_state = torch.FloatTensor(lidar_state.reshape(1, -1)).to(self.device)
            position_state = torch.FloatTensor(position_state.reshape(1, -1)).to(self.device)
