...
n_laser = 1800
file_prefix = '/home/zw/expand_disk/ubuntu/RNN_RL/complex/seed_1'
file_name = sorted(glob.glob(file_prefix + '/evaluation_episodes/*.npz'))
# print(file_name[-1])
log_env = np.load(file_name[-7])
...
static_obs = True
if static_obs:
    static_obstacles = log_env['rectangles']
    static_obs_num = static_obstacles.shape[1]
...
#     if static_obs:
#         for k in range(static_obs_num):
#             ax.add_patch(Rectangle((static_obstacles[i][k][0], static_obstacles[i][k][1]), 
#                                     static_obstacles[i][k][2], static_obstacles[i][k][3],
#                                     facecolor='#EB70AA',
#                                     fill=True))
...
with writer.saving(fig, "ego_complex_3_2.mp4", 100):
    for i in range(steps):
        ax.clear()
        ax.set_xlabel('x/m', fontproperties = 'Times New Roman', fontsize=24)
        ax.set_ylabel('y/m', fontproperties = 'Times New Roman', fontsize=24) 
